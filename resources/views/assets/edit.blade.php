    ...
    <div class="py-12">
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                ...
                <p class="mb-4 text-sm text-gray-500">Kode Asset: <strong>{{ $asset->asset_code }}</strong> (tidak bisa diubah)</p>
                ...
                <form action="{{ route('assets.update', $asset) }}" method="POST">
                    ...
                    <div class="mb-4">
                        ...
                        <select name="location_area_id" id="location_area_id" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
                            @foreach ($locationAreas as $area)
                                <option value="{{ $area->id }}" @selected(old('location_area_id', $asset->location_area_id) == $area->id)>
                    ...
                    <div class="mb-4">
                        ...
                        <select name="status" id="status" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
                            ...
                            <option value="retired" @selected(old('status', $asset->status) == 'retired')>Retired</option>
                        </select>
                    </div>
                    <div class="mb-4">
                        <label for="description" class="block text-sm font-medium text-gray-700">Deskripsi (opsional)</label>
                        <textarea name="description" id="description" rows="3"
                                  class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">{{ old('description', $asset->description) }}</textarea>
                    </div>
