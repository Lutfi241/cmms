<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">Edit Floor</h2>
    </x-slot>
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                ...
                <form action="{{ route('floors.update', $floor) }}" method="POST">
                    ...
                    <div class="mb-4">
                        ...
                        <select name="building_id" id="building_id" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
                            @foreach ($buildings as $building)
                                <option value="{{ $building->id }}" @selected(old('building_id', $floor->building_id) == $building->id)>
                                    {{ $building->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="name" id="name" value="{{ old('name', $floor->name) }}"
                               class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
                    </div>
                    <div class="mb-4">
                        <label for="code" class="block text-sm font-medium text-gray-700">Kode</label>
                        <input type="text" name="code" id="code" value="{{ old('code', $floor->code) }}"
                               class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
                    </div>
